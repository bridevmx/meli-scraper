...
const getApiBaseUrl = () => {
  // Use API_BASE_URL from .env, or fall back to localhost with the current port
...
router.get('/load-test-stream/offers', (req, res) => {
  ...
  const testConfig = {
    url: `${getApiBaseUrl()}/api/scraper/offers?maxPages=1`,
...
router.get('/load-test-stream/search', (req, res) => {
  ...
  const testConfig = {
    url: `${getApiBaseUrl()}/api/scraper/search?q=laptop&maxPages=1`,
    numRequests: 10,
  };
  runRealtimeTest(res, testConfig);
});
...
router.post('/load-test-stream/product', (req, res) => {
  ...
  const testConfig = {
    url: `${getApiBaseUrl()}/api/scraper/product`,
    method: 'POST',
    body: {
